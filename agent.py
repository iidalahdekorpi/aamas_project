...
class Agent:
    ...
    '''
    Agent for the random apples environment
    '''
    ...
    def __init__(self, id, grid_size,n_apples,n_agents,maxlevel, agentType, NA = 6, alpha = 0.1, gamma = 0.9):
        ...
        if self.agentType == 'independent':
            ...
            self.Q = np.ones(((self.NL*maxlevel)**(n_agents + n_apples),NA))*2
        elif self.agentType == 'central':
            self.Q = np.ones(((self.NL*maxlevel) ** (n_agents + n_apples),NA**n_agents))*2
...
class Agent2:
    ...
    '''
    Agent2 for the fixed apples environment
    '''

    def __init__(self, id, grid_size,n_apples,n_agents, maxlevel, NA = 6, alpha = 0.1, gamma = 0.9, agentType = 'independent'):
        self.alpha = alpha
        self.gamma = gamma
        self.id = id
        self.agentType = agentType
        ...
        if self.agentType == 'independent':
            self.Q = np.ones(((grid_size ** 2),NA))*2
        elif self.agentType == 'observ':
            self.Q = np.ones(((grid_size ** 2) ** n_agents,NA))*2
        elif self.agentType == 'central':
            self.Q = np.ones(((grid_size ** 2) ** n_agents,NA**n_agents))*2
        if self.agentType == 'JALAM':
            self.Q = np.ones(((grid_size ** 2) ** n_agents, NA, NA)) * 2
            self.C = np.zeros(((grid_size ** 2) ** n_agents, NA, NA)) + 0.001
    ...
    def observ2state(self, x):
        ...
        n_apples = self.n_apples - apples_decremented
        if self.agentType == 'independent':
            return  states[n_apples]
        elif self.agentType == 'observ' or self.agentType == 'central' or self.agentType == 'JALAM':
            shape = tuple(self.grid_size ** 2 for _ in range(self.n_agents))

            return np.ravel_multi_index(states[-self.n_agents:],shape)
    ...
    def update(self,x,nx,a,r):
        ...
        x = list(map(int, x))
        if self.agentType == 'central':
            shape = tuple(self.NA for _ in range(self.n_agents))
            ai = np.ravel_multi_index(a,shape)
            self.Q[xi,ai] += self.alpha * (np.sum(r) + self.gamma * np.max(self.Q[nxi,:]) - self.Q[xi,ai])
        elif self.agentType == 'JALAM':
